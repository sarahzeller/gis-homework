...
if (!(here("output/nightlights_sf.rds") |> exists())) {
...
# crop raster to Tanzania
if (! (here("output/landcover_tanzania.tif") |> exists())) {
  here("output/tri/LandCover_OneKilometerData/LANDCOVER1k.tif") |>
    raster::raster() |> 
    raster::crop(tanzania_whole |> st_transform("ESRI:54010")) |> 
    raster::writeRaster(here("output/landcover_tanzania.tif"), overwrite = TRUE)
}

# read in
landcover <- here("output/landcover_tanzania.tif") |> 
  terra::rast() |> 
  terra::project("epsg:4326")
...
if (! (here("output/ghsl.tif") |> exists())) {
  # download
...
tri_lightnings_zonal <- zonal(tri,
                              nightlights_vect,
                              fun = "mean", na.rm = TRUE)

saveRDS(tri_lightnings_zonal,
        here("output/tri_lightnings_zonal.rds"))
...
ghsl_lightnings_zonal <- zonal(ghsl,
                               nightlights_vect,
                               fun = "mean", na.rm = TRUE)

saveRDS(ghsl_lightnings_zonal,
        here("output/ghsl_lightnings_zonal.rds"))
...
data <- nightlights_sf |> 
  ...
  cbind(tri_lightnings_zonal) |> 
  cbind(ghsl_lightnings_zonal) |> 
  mutate(number_antennas = ntl_antennas_vector) |> 
  mutate(log_number_antennas = log(ntl_antennas_vector + 1e-10))
```

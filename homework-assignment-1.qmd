...
coast <- ne_coastline(returnclass = "sf") |> 
  ...
  st_intersection(tanzania_whole |> 
                    st_buffer(2)) # buffer b/c overlap is not complete
```
...
if (here("output/nn_roads.rds") |> file.exists()) {
  nn_poly <- readRDS("output/nn_roads.rds")
} else {
  nn_roads <- st_nn(open_cell,
                   roads,
                   returnDist = TRUE)
  saveRDS(nn_poly, 
          "output/nn_roads.rds")
...
# extract distances
dists_roads <- sapply(nn_roads[[2]], "[", 1)
```

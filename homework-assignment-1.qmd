...
We use the `fixest` package to run regressions with the amassed data set.
We then use the `modelsummary` package to show the regression output.
...
formula <- reformulate(open_cell_w_distances |>
                         ...
                         select(starts_with("dist_")) |> # select all distances
                         select(-dist_coast_km) |> # de-select coastline since it is colinear with border
                         names(),
                       "created_year")
